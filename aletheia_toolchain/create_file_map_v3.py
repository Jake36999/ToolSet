...
BUILTIN_PROFILES = {
    ...
    "python_project": {
        "include_exts": [".py", ".toml", ".md", ".json", ".yaml", ".yml", ".cfg", ".ini"],
        "exclude_dirs": [
    ...
    "polyglot_runtime": {
        ...
        "exclude_dirs": [
            ...
            ".next", "build", "dist", "coverage", ".cache",
        ],
        "max_file_size": DEFAULT_MAX_FILE_SIZE,
        "suspicious_dirs": DEFAULT_SUSPICIOUS_DIRECTORIES,
    },
    "training_pipeline": {
        "include_exts": [".py", ".yaml", ".yml", ".json", ".toml", ".md", ".txt", ".sh"],
        "exclude_dirs": [
            ".git", ".venv", "venv", "__pycache__", "output", "outputs",
            "runs", "checkpoints", "checkpoint", "models", "weights",
    ...
    "node_python_runtime": {
        ...
        "exclude_dirs": [
            ...
            ".next", "dist", "build", "coverage", ".pytest_cache", ".cache",
        ],
        "max_file_size": DEFAULT_MAX_FILE_SIZE,
        "suspicious_dirs": DEFAULT_SUSPICIOUS_DIRECTORIES,
    },
...
def build_health_report(
    ...
    if report["pollution_count"] > 0:
        report["status"] = "WARN" if not fail_on_pollution else "BLOCK"
    else:
        report["status"] = "PASS"
    return report
...
def main() -> int:
    ...
    if args.fail_on_pollution and stats["polluted_files"]:
        return 2
    return 0
