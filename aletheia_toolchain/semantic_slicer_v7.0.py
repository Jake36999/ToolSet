"""
Agnostic Semantic Slicer v7.0 - Research-Grade LLM Context Engine (Config-Integrated)
...
Phase 6 additions over v6.0 baseline:
    - Config/profile integration via aletheia_tool_core.config helpers.
      New flags: --config, --task-profile, --validate-only, --allow-path-merge-with-manifest
    - Safer manifest semantics: combining positional paths with --manifest now BLOCKs
      by default; requires explicit --allow-path-merge-with-manifest to proceed.
    - --validate-only mode: validates config/profile and reports planned scan strategy
      without producing a full slicer bundle.
    - Profile settings wired: exclude_dirs (merged into ignore list), include_exts
      (post-collection whitelist filter), deterministic default, append_rules default,
      max_file_size override, output format default.
    - WorkspacePackager accepts per-invocation max_file_size (from profile/project config).

v6.0 capabilities preserved:
    - All original CLI flags unchanged.
    - Manifest scans, Git-diff scans, focus/depth traversal.
    - Deterministic output, JSON/text output, heatmap diagnostics, explain mode.
    - Risk/patch maps, Layer X uncertainty reporting, state mutation maps.
    - Idempotency safeguard against recursive bundle slicing.
"""
...
# Phase 5 config helpers — optional import so v7 still degrades gracefully if
# run outside the aletheia_toolchain package tree.
try:
    ...
    _CONFIG_HELPERS_AVAILABLE = True
except ImportError:  # pragma: no cover
    _CONFIG_HELPERS_AVAILABLE = False
...
BUNDLE_SCHEMA_VERSION = "agnostic_bundle_v7.0_phase6"
SLICER_VERSION = "v7.0"
...
class WorkspacePackager:
    def __init__(
        ...
        # Phase 6: per-invocation overrides from config/profile
        ...
        self.max_file_size = max(1, max_file_size)  # Phase 6: profile/config override
    ...
    def _process_single_file(self, path_obj: pathlib.Path, analyzer: "PolyglotAnalyzer") -> Dict[str, Any]:
        try:
            ...
            if size > self.max_file_size: return {"status": "skipped", "skip_reason": "oversize", "ext": path_obj.suffix}
...
# MAIN
# ============================================================================
def _load_and_apply_config(args: argparse.Namespace) -> Dict[str, Any]:
    """Phase 6 helper: load + validate config, resolve profile, return profile_settings dict.

    Returns an empty dict when no --config is supplied, so callers can unconditionally
    do ``profile_settings.get(key)``.

    Raises SystemExit(1) on any config/profile error so the caller's flow stays clean.
    """
    if not args.config:
        return {}

    if not _CONFIG_HELPERS_AVAILABLE:
        sys.exit(
            "ERROR: aletheia_tool_core is not importable from this working directory. "
            "Run from the aletheia_toolchain/ root or install the package."
        )
    ...
    if args.task_profile:
        try:
            profile_settings = resolve_profile(cfg_data, args.task_profile)
        except ConfigError as exc:
...
def _print_validate_only_report(
    args: argparse.Namespace,
    profile_settings: Dict[str, Any],
    effective_ignore_dirs: List[str],
    effective_max_file_size: int,
) -> None:
    """Phase 6: print a human-readable strategy report and exit without slicing."""
    lines = [
        f"=== semantic_slicer {SLICER_VERSION} — validate-only mode ===",
        "",
        "[Config]",
        f"  --config         : {args.config or '(none)'}",
        f"  --task-profile   : {args.task_profile or '(none)'}",
        f"  Config helpers   : {'available' if _CONFIG_HELPERS_AVAILABLE else 'NOT available — install aletheia_tool_core'}",
    ]

    if profile_settings:
        lines += ["", "[Profile settings resolved]"]
        for k, v in sorted(profile_settings.items()):
            lines.append(f"  {k}: {v}")
    else:
        lines += ["", "[Profile settings] (none)"]

    lines += [
        "",
        "[Scan inputs]",
        f"  positional paths : {args.paths or '(none)'}",
        f"  --manifest       : {args.manifest or '(none)'}",
        f"  --git-diff       : {args.git_diff}",
        f"  --base-dir       : {args.base_dir}",
        "",
        "[Effective scan strategy]",
        f"  ignore_dirs      : {sorted(effective_ignore_dirs)}",
        f"  include_exts     : {profile_settings.get('include_exts', '(all — no whitelist)')}",
        f"  max_file_size    : {effective_max_file_size:,} bytes",
        f"  deterministic    : {args.deterministic or bool(profile_settings.get('deterministic', False))}",
        f"  append_rules     : {args.append_rules or bool(profile_settings.get('append_rules', False))}",
        f"  output format    : {args.format}",
        f"  workers          : {args.workers}",
        "",
        "[Safety flags]",
        f"  --allow-path-merge-with-manifest : {args.allow_path_merge_with_manifest}",
        "",
        "validate-only check passed. No bundle produced.",
    ]
    print("\n".join(lines))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"Agnostic Semantic Slicer {SLICER_VERSION} (Config-Integrated)"
    )
    # ------------------------------------------------------------------ v6 flags (preserved)
    ...
    parser.add_argument(
        "--config",
        help="Path to a semantic_project_config JSON file (Phase 5 schema).",
    )
    parser.add_argument(
        "--task-profile",
        help="Named profile to resolve from --config. Requires --config.",
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate config/profile and print the planned scan strategy; do not produce a bundle.",
    )
    parser.add_argument(
        "--allow-path-merge-with-manifest",
        action="store_true",
        help="Allow positional paths alongside --manifest. Without this flag, combining both is an error.",
    )

    args = parser.parse_args()
    setup_logging(args.verbose)

    # ------------------------------------------------------------------ safety: manifest + paths
    if args.manifest and args.paths and not args.allow_path_merge_with_manifest:
        sys.exit(
            "ERROR: Combining positional paths with --manifest is unsafe by default.\n"
            "       Use --allow-path-merge-with-manifest to override this safety block."
        )

    # ------------------------------------------------------------------ Phase 6: config / profile
    profile_settings = _load_and_apply_config(args)

    # Build effective ignore-dirs: union of CLI/default + profile exclude_dirs
    effective_ignore_dirs: List[str] = list(
        set(args.ignore_dirs) | set(profile_settings.get("exclude_dirs") or [])
    )

    # Resolve effective max_file_size via precedence helper (or fall back without it)
    if _CONFIG_HELPERS_AVAILABLE and profile_settings:
        _resolved = resolve_precedence(
            cli_overrides=None,
            profile_settings={"max_file_size": profile_settings.get("max_file_size")},
            project_defaults=None,
            builtin_defaults={"max_file_size": MAX_FILE_SIZE_BYTES},
        )
        effective_max_file_size: int = int(_resolved.get("max_file_size", MAX_FILE_SIZE_BYTES))
    else:
        effective_max_file_size = MAX_FILE_SIZE_BYTES

    # Boolean profile defaults (profile wins over built-in False; explicit CLI True wins)
    effective_deterministic: bool = args.deterministic or bool(
        profile_settings.get("deterministic", False)
    )
    effective_append_rules: bool = args.append_rules or bool(
        profile_settings.get("append_rules", False)
    )
    # Output format: CLI default wins; if profile specifies one and CLI is still at default, use it
    effective_format: str = args.format
    if profile_settings.get("format") in ("text", "json") and args.format == "text":
        effective_format = str(profile_settings["format"])

    # ------------------------------------------------------------------ --validate-only exit
    if args.validate_only:
        _print_validate_only_report(args, profile_settings, effective_ignore_dirs, effective_max_file_size)
        sys.exit(0)
        ...
        for p_str in args.paths:
            ...
            if p.is_file():
                ...
                for root, dirs, files in os.walk(p):
                    ...
                    for f in files:
                        add_target_file(pathlib.Path(root) / f)
    ...
    # Phase 6: apply include_exts whitelist filter if profile specifies one
    profile_include_exts = profile_settings.get("include_exts")
    if profile_include_exts:
        allowed_set = {e.lower() for e in profile_include_exts}
        before_count = len(target_files)
        target_files = [f for f in target_files if f.suffix.lower() in allowed_set]
        logger.info(
            f"Profile include_exts filter: {before_count} → {len(target_files)} files "
            f"(whitelist: {sorted(allowed_set)})"
        )
    ...
    packager = WorkspacePackager(
        ...
        depth=args.depth,
        append_rules=effective_append_rules,
    ...
    bundle = packager.run(effective_format, heatmap=args.heatmap, explain=args.explain)

    out_path = (
        pathlib.Path(args.output)
        if args.output
        else base_path / f"{base_path.name}_bundle.{'json' if effective_format == 'json' else 'txt'}"
    )
